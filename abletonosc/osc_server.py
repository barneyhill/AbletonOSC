...
class OSCServer:
    ...
    def process_message(self, message):
        ...
        if message.address in self._callbacks:
            ...
            if rv is not None:
                assert isinstance(rv, tuple)
                responses.append((message.address, rv))
        elif "*" in message.address:
            regex = message.address.replace("*", "[^/]+")
            for callback_address, callback in self._callbacks.items():
                if re.match(regex, callback_address):
                    try:
                        ...
                        if rv is not None:
                            assert isinstance(rv, tuple)
                            responses.append((callback_address, rv))
                    except (ValueError, AttributeError):
                        # Handling specific exceptions as in the original implementation
                        continue
        else:
            self.logger.error("AbletonOSC: Unknown OSC address: %s" % message.address)
        return responses
    ...
    def process_bundle(self, bundle):
        ...
        for i in bundle:
            if OscBundle.dgram_is_bundle(i.dgram):
                responses.extend(self.process_bundle(i))
            else:
                responses.extend(self.process_message(i))
        return responses

    def parse_bundle(self, data):
        if OscBundle.dgram_is_bundle(data):
            try:
                bundle = OscBundle(data)
                return self.process_bundle(bundle)
            except ParseError:
                self.logger.error("AbletonOSC: Error parsing OSC bundle.")
        else:
            try:
                message = OscMessage(data)
                return self.process_message(message)
            except ParseError:
                self.logger.error("AbletonOSC: Error parsing OSC message.")
